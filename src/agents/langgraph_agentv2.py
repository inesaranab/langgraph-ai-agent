...
class LangGraphAgent:
    ...
    def __init__(self, config: AppConfig, openai_api_key: Optional[str] = None, tavily_api_key: Optional[str] = None):
        ...
        # Validate required API keys and ensure they are strings
        ...
        # Ensure API keys are strings (fix for SecretStr validation)
        if not isinstance(self.openai_api_key, str):
            raise ValueError(f"OpenAI API key must be a string, got {type(self.openai_api_key)}")
        
        if not isinstance(self.tavily_api_key, str):
            raise ValueError(f"Tavily API key must be a string, got {type(self.tavily_api_key)}")
